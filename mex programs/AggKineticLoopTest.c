...
double SumArrayRow(double *b, int row, int ColStart, int rowSize, int colSize)
{
    ...
    */
    ...
    for (i = iStart; i < rowSize*colSize; i += rowSize)
    {
        tot += b[i];
    }
    return tot;
...
void mexFunction( int nlhs, mxArray *plhs[],int nrhs, const mxArray *prhs[])
{
    ...
    int agn1,agn2,agn3,ii,jj,aRowSize,aColSize;
    ...
    for (i = 1; i < *m; ++i)
    {
        ...
        *yn += dy1 - dy2;                // Update number of free particles (aggregates and monomers)
        ...
        aa[0] = a[ii];
        // a2 = clpf*y[0]*y(2:(n-1));
        for (ii = FindPos(1,rst[1],aRowSize); ii<=FindPos(1,re[1],aRowSize); ii += aRowSize)
        {
			jj = ii-rst[1]+1;
            a[ii] = (*clpf)*y[0]*y[jj];
        }
        ...
        // a5 = cap*g0*y;
        for (ii=FindPos(4,rst[4],aRowSize); ii<=FindPos(4,re[4],aRowSize); ii += aRowSize)
        {
			jj = ii-rst[4]+1;
			a[ii] = (*cap)*g0*y[jj];    
        }
        ...
        // a6 = clpm*y[0]*g(1:(n-1));
        for (ii=FindPos(5,rst[5],aRowSize); ii<=FindPos(5,re[5],aRowSize); ii += aRowSize)
        {
			jj = ii-rst[5];
            a[ii] = (*clpm)*y[0]*g[jj];
        }
